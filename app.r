...
ui <- 
  function(request){dashboardPagePlus( collapse_sidebar = TRUE,
                                         ...
                                         left_menu =
                                           tagList(
                                             dropdownBlock(
                                               ...
                                               column(width=12,
                                                      textInput(inputId = "bookmarkcim", label="Mentett riport neve", placeholder = "Tetszőleges elnevezés"),
                                                      bookmarkButton(id = "bookmarkBtn", icon=icon("save"), label = "Mentés"),
                                                      br()
                                               )
                                       ...
                                       body = dashboardBody( style = "background-color:#555a60;",
                                                                        ...
                                                                        tabItem(tabName = "szurok", h2("Szűrők"),
                                                                                ...
                                                                                boxPlus( width=12, collapsed=T, collapsible=T, closable=F, title="Tengely- és értékszűrők", status="warning", solidHeader = T,
                                                                                         ...
                                                                                         conditionalPanel(
                                                                                           ...
                                                                                           conditionalPanel(condition = "input.datumagr == true", selectInput(
                                                                                             "osszevontdatumsz", "Dátumszűrő",
                                                                                             c("Éves összevonás" = "currentyear",
                                                                                               "Negyedéves összevonás"="currentQ",
                                                                                               "Havi összevonás"="currentmonth",
                                                                                               "Heti összevonás"="currentweek")
                                                                                           ))
                                                                                         ),
                                                                                ...
                                                                                boxPlus( width=12, collapsed=T, collapsible=T, closable=F, title="Adatfeliratok", status="warning", solidHeader = T,
                                                                                         fluidRow(
                                                                                           ...
                                                                                           column(
                                                                                             ...
                                                                                             numericInput("label_format_decimal", "Tizedesjegyek:", "0", min=0, max=3)
                                                                                             #,
                                                               ...
                                                               column(8,
                                                                      ...
                                                                      div(
                                                                        ...
                                                                        boxPlus( width = 12, status = "danger",
                                                                                 ...
                                                                                 tabsetPanel(
                                                                                   id = "chart_tabset",
                                                                                   tabPanel('Táblázat', value="table", DT::DTOutput("tabletab")),
                                                                                   tabPanel('Halmozott oszlop', value="stacked_bar", plotly::plotlyOutput('barplot')),
                                                                                   tabPanel('Csoportosított oszlop', value="clustered_bar", plotly::plotlyOutput('barplotclust')),
                                                                                   tabPanel('Halmozott sávdiagram', value="stacked_sav", plotly::plotlyOutput('savplot')),
                                                                                   tabPanel('Csoportosított sávdiagram', value="clustered_sav", plotly::plotlyOutput('savplotclust')),
                                                                                   tabPanel('Pontdiagram', value="scatter", plotly::plotlyOutput('pointplot')),
                                                                                   tabPanel('Területdiagram', value="area", plotly::plotlyOutput('areaplot')),
                                                                                   tabPanel('Vonaldiagram', value="line", plotly::plotlyOutput('lineplot')),
                                                                                   tabPanel('Kördiagram', value="pie", plotly::plotlyOutput('pieplot')),
                                                                                   tabPanel('Fánkdiagram', value="donut", plotly::plotlyOutput('donutplot'))
...
server <- function(input, output, session) {
  ...
  plot_fuggveny <-
    function(plot_type = "base",
             ...
             x = merged_colnames[which(colnames(abra_df_react()) ==ifelse(input$datumagr==FALSE,input$tengelyvalaszt, input$osszevontdatumsz))],
  ...
  number_formatting <- function(szam, egyseg, tizedesjegy, szazalek) {
    ...
    #tizedesjegy
    szam <- format(round(szam, digits = tizedesjegy), nsmall = tizedesjegy)
  ...
  proba_plotly <-
    ...
    )
    {
      ...
      abra_df <- abra_df_react()
      abra_df[3] <- lapply(abra_df[3], number_formatting, egyseg = input$label_format_unit, tizedesjegy = input$label_format_decimal, szazalek = F)#input$label_format_percent) #fent van összerakva!!!
      ...
      finished_plot <- 
        if (chart_type == "bar") {
          ...
          plotly::plot_ly(
            ...
            x = ~
              if (input$tengelyvalaszt == "DATUM") {
                ...
                #get(x)
                reorder ( factor(get(x), levels = unique( c(as.character(get(x)))) ) , -get(y), FUN = sum )
          ...
          plotly::plot_ly(
            ...
            sort = T,
            textfont = list(color = input$label_color, size = input$label_size, family = input$font_type),
  ...
  active_chart <- function(){
    if(input$chart_tabset=="stacked_bar") {
      ...
      active_plot <- final_chart(chart_type = "bar", mode = "stack") #csak hogy legyen valami
    }
  ...
  output$treemapplot <- renderPlot({
    ggplot(abra_df_react(),
           aes(
             ...
             fill = get(colnames(abra_df_react())[which(colnames(abra_df_react()) ==ifelse(input$datumagr==FALSE,input$tengelyvalaszt, input$osszevontdatumsz))]),
             label = get(colnames(abra_df_react())[which(colnames(abra_df_react()) == input$kategoriavalaszt)]),
             subgroup = get(colnames(abra_df_react())[which(colnames(abra_df_react()) ==ifelse(input$datumagr==FALSE,input$tengelyvalaszt, input$osszevontdatumsz))])
           )) +
      geom_treemap() +
