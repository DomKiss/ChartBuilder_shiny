                                                                        ...
                                                                        tabItem(tabName = "szurok", h2("Szűrők"),
                                                                                ...
                                                                                boxPlus( width=12, collapsed=T, collapsible=T, closable=F, title="Tengely- és értékszűrők", status="warning", solidHeader = T,
                                                                                         ...
                                                                                         conditionalPanel("input.tengelyvalaszt=='DATUM'", 
                                                                                                          selectInput("datumszuro", 
                                                                                                                      "Dátumszűrő:", 
                                                                                                                      c("Év utolsó napja" = "UNAP_EV", 
                                                                                         ...
                                                                                         conditionalPanel("input.tengelyvalaszt=='DATUM'", materialSwitch(inputId = "datumagr", label = "Időben összevonva", status = "warning")),
...
server <- function(input, output, session) {
  ...
  proba_plotly <-
    ...
    )
    {
      ...
      abra_df <- abra_df_react()
      abra_df[3] <- lapply(abra_df[3], number_formatting, egyseg = input$label_format_unit, tizedesjegy = input$label_format_decimal, szazalek =input$label_format_percent) #fent van összerakva!!!
  ...
  final_chart <- function(chart_type, mode)
    { 
      ...
      if (chart_type == "bar" & mode == "stack") {
        ...
        proba_plotly(chart_type="scatter") %>% design()
      }
    } 
  ...
  output$lineplot <- plotly::renderPlotly({
    ...
    final_chart(chart_type = "scatter", mode = "lines")
  })
  ...
  output$downloadPlot <- downloadHandler(
    ...
    content = function(file) {
          # orca(
          #   plot_ly(mtcars, x = ~factor(vs), y = ~mpg) %>%  add_trace(type = "violin"),
          #   "C:/Users/hunter5/Documents/ChartBuilder_shiny",
          #   scale=5
          # )
